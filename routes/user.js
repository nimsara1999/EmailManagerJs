...
router.post("/signup", (req, res) => {
  ...
  connection.query(query, [user.email], async (err, results) => {
    ...
    if (!err) {
      if (results.length <= 0) {
        query =
          "INSERT INTO recipient(email,user_password,recip_name,nick_name,post_id,birthday,state) VALUES (?,?,?,?,?,?,0)";
        var value = user.user_password;
...
router.post("/login", (req, res) => {
  const user = req.body;
  query =
    "SELECT recipient.email,recipient.user_password,post.post,state FROM recipient LEFT JOIN post ON recipient.post_id=post.post_id  WHERE email=?";
  connection.query(query, [user.email], async (err, results) => {
    if (!err) {
    ...
    if (!err) {
      if (results.length <= 0) {
        ...
        var mailOptions = {
          ...
          html:
            "<p><b>Your Login Details of EmailManager</b><br><b>Email: </b>" +
            results[0].email +
            "<br><b>Password: </b>" +
            results[0].user_password +
        ...
        return res
          .status(200)
          .json({ message: "Password sent successfuly to your email." });
      }
    } else {
      return res.status(500).json(err);
    }
...
router.get("/get", auth.authenticateToken, checkRole.checkRole, (req, res) => {
  var query =
    "SELECT recipient.email,post.post,status FROM recipient LEFT JOIN post ON recipient.post_id=post.post_id  WHERE post.post!='administrator'";
