...
router.get('/detailsUser',auth.authenticateToken,(req,res,next)=>{
    ...
    var query = "SELECT count(message_id) AS inboxCount FROM message_data WHERE toAddr=?";
...
router.get('/detailsAddmin',auth.authenticateToken,checkRole.checkRole,(req,res)=>{
    var resCount;
